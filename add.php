...
if ( !$connection ) {
    ...
    if ( 'addManager' == $action ) {
    ...
    } elseif ( 'addMedicen' == $action ) {
        $Mname = $_REQUEST['Mname'] ?? '';
        $MFdate = $_REQUEST['MFdate'] ?? '';
        $EXdate = $_REQUEST['EXdate'] ?? '';
        $Rate = $_REQUEST['Rate'] ?? '';
        

        if ( $Mname && $MFdate && $EXdate && $Rate ) {
            // $hashPassword = password_hash( $password, PASSWORD_BCRYPT );
            $query = "INSERT INTO medicen(Mname,MFdate,EXdate,Rate) VALUES ('{$Mname}','$MFdate','$EXdate','$Rate')";
            mysqli_query( $connection, $query );
            header( "location:index.php?id=allMedicen" );
        }
    } elseif ( 'updateMedicen' == $action ) {
        $id = $_REQUEST['id'] ?? '';
        $Mname = $_REQUEST['Mname'] ?? '';
        $MFdate = $_REQUEST['MFdate'] ?? '';
        $EXdate = $_REQUEST['EXdate'] ?? '';
        ...
        if ( $Mname && $MFdate && $EXdate && $Rate ) {
            $query = "UPDATE medicen SET Mname='{$Mname}', MFdate='{$MFdate}', EXdate='$EXdate', Rate='$Rate' WHERE id='{$id}'";
            mysqli_query( $connection, $query );
            header( "location:index.php?id=allMedicen" );
        }
    }
